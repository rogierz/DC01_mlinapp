...
def process_mask(mask):
    ...
    if den <= 0:
        den == 1
    color_mask = (plt.cm.jet(single_ch_mask/den)[:,:,:3] * 255.0).astype(np.uint8)

    # save output
    mask_img = Image.fromarray(color_mask)
    return mask_img

def apply_mask_on_image(img, mask, out_path, true_mask=None, id=0):
    ...
    mask_img.save(os.path.join(out_path,f'{id:02d}_predicted_mask.png'))

    composite = Image.composite(img, mask_img, Image.new("L", mask_img.size, 128))
    composite.convert('RGB').save(os.path.join(out_path, f'{id:02d}_predicted_mask_on_img.png'))

    if true_mask is not None:
        true_mask_img = process_mask(true_mask)
        true_mask_img.save(os.path.join(out_path, f'{id:02d}_true_mask.png'))
        
        true_composite = Image.composite(img, true_mask_img, Image.new("L", true_mask_img.size, 128))
        true_composite.convert('RGB').save(os.path.join(out_path, f'{id:02d}_true_mask_on_img.png'))
